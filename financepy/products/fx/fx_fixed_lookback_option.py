    ...
    def value(self,
        ...
        if self._option_type == option_types.EUROPEAN_CALL:
            ...
            if k > smax:
                ...
                if s0 == k:
                    term = -normal_cdf(d1 - 2.0 * b * sqrt(t) / v) + expbt * normal_cdf(d1)
                elif s0 < k and w > 100:
                    term = expbt * normal_cdf(d1)
                else:
                    term = (-(s0 / k)**(-w)) * normal_cdf(d1 - 2.0 *
                                                          b * sqrt(t) / v) + expbt * normal_cdf(d1)
                ...
                if s0 == smax:
                    term = -normal_cdf(e1 - 2.0 * b * sqrt(t) / v) + expbt * normal_cdf(e1)
                elif s0 < smax and w > 100:
                    term = expbt * normal_cdf(e1)
                else:
                    term = (-(s0 / smax)**(-w)) * \
                           normal_cdf(e1 - 2.0 * b * sqrt(t) / v) + expbt * normal_cdf(e1)
            ...
            if k >= smin:
                ...
                if s0 == smin:
                    term = normal_cdf(-f1 + 2.0 * b * sqrt(t) / v) - expbt * normal_cdf(-f1)
                elif s0 > smin and w < -100:
                    term = -expbt * normal_cdf(-f1)
                else:
                    term = ((s0 / smin)**(-w)) * normal_cdf(-f1 + 2.0 *
                                                            b * sqrt(t) / v) - expbt * normal_cdf(-f1)
