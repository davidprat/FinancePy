...
from .calendar import (calendar, calendar_types)
from .calendar import (bus_day_adjust_types, date_gen_rule_types)
from .frequency import (annual_frequency, frequency_types)
...
class Schedule:
    ...
    a termination date. Dates are adjusted to a provided calendar.

    The zeroth element is the previous coupon date (PCD) and the first element is the
    ...
    def __init__(self,
        ...
        if effective_date >= termination_date:
            raise finpy_error("Effective date must be before termination date.")
        ...
        if first_date is None:
            ...
            if first_date > effective_date and first_date < termination_date:
                ...
                raise finpy_error("First date must be after effective date and" +
        ...
        if next_to_last_date is None:
            ...
            if next_to_last_date > effective_date and next_to_last_date < termination_date:
                ...
                raise finpy_error("Next to last date must be after effective date and" +
                               " before termination date")
    ...
    def _generate(self):
        ...
        if self._date_gen_rule_type == date_gen_rule_types.BACKWARD:
            ...
            _date = unadjusted_schedule_dates[flow_num - 1]
            self._adjusted_dates.append(_date)
            ...
            for i in range(1, flow_num - 1):
                _date = calendar.adjust(unadjusted_schedule_dates[flow_num - i - 1],
            ...
            for i in range(1, flow_num):
                _date = calendar.adjust(unadjusted_schedule_dates[i],
                                        self._bus_day_adjust_type)
        ...
        if len(self._adjusted_dates) < 2:
            raise finpy_error("Schedule has two dates only.")

        prev_dt = self._adjusted_dates[0]
        for _date in self._adjusted_dates[1:]:

            if _date == prev_dt:
                raise finpy_error("Two matching dates in schedule")

            if _date < prev_dt:  # Dates must be ordered
                raise finpy_error("Dates are not monotonic")
    ...
    def __repr__(self):
        ...
        _str = label_to_string("OBJECT TYPE", type(self).__name__)
        _str += label_to_string("EFFECTIVE DATE", self._effective_date)
        _str += label_to_string("END DATE", self._termination_date)
        _str += label_to_string("FREQUENCY", self._freq_type)
        _str += label_to_string("CALENDAR", self._calendar_type)
        _str += label_to_string("BUSDAYRULE", self._bus_day_adjust_type)
        _str += label_to_string("DATEGENRULE", self._date_gen_rule_type)
        _str += label_to_string("ADJUST TERM DATE", self._adjust_termination_date)
        _str += label_to_string("END OF MONTH", self._end_of_month_flag, "")

        return _str

    def _print(self):
        """ Print out the details of the schedule and the actual dates.

        This can be used for providing transparency on schedule calculations.
        """
        print(self)
